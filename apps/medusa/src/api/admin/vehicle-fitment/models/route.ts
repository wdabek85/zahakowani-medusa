...
import { createVehicleModelSchema } from "../../../../validators/vehicle-fitment"

/** POST /admin/vehicle-fitment/models — create vehicle model. */
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    const [model] = await service.createVehicleModels([input])
    res.status(201).json({ vehicle_model: model })
  } catch (err) {
