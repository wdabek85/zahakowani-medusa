...
import { updateVehicleModelSchema } from "../../../../../validators/vehicle-fitment"
...
export async function GET(
  ...
  try {
    const model = await service.retrieveVehicleModel(req.params.id)
    res.json({ vehicle_model: model })
  } catch {
    res.status(404).json({ error: `VehicleModel "${req.params.id}" not found` })
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    const service = req.scope.resolve<VehicleFitmentService>(VEHICLE_FITMENT_MODULE)
    const model = await service.updateVehicleModels({ id: req.params.id, ...input })
    res.json({ vehicle_model: model })
  } catch (err) {
...
export async function DELETE(
  ...
  const service = req.scope.resolve<VehicleFitmentService>(VEHICLE_FITMENT_MODULE)
  try {
    await service.deleteVehicleModels([req.params.id])
